...
vector<Trajectory> Solver::solve(vector<Trajectory> droneWpts, bool initial) {
    ...
    vector<double> lba;
    vector<double> uba;
    cout<<"nx: "<<nx<<" nc: "<<nc<<" ts: "<<tList.size()<<endl;
    double t0 = tList[0];
    ...
    cout<<"lba: "<<lba.size()<<endl<<"uba: "<<uba.size()<<endl;
    real_t g[nx];
    fill(g, g+nx, 0);
    real_t* xOpt = new real_t[nx];

    // if(initial) {
        // sqp = new SQProblem(nx, nc);

    QProblem qp(nx, nc);
    Options options;
    options.setToMPC();
    qp.setOptions(options);
    returnValue rv = qp.init(H_r,g,A_r,nullptr,nullptr,lb_r,ub_r, nWSR);
    cout<<"returnValue: "<<rv<<endl;
    // }
    // else {
        // returnValue rv = sqp->hotstart(H_r,g,A_r,nullptr,nullptr,lb_r,ub_r, nWSR, 0);
    // }
    qp.getPrimalSolution(xOpt);
    // delete qp;
    cout<<"Optimization problem solved"<<endl;
    ...
    cout<<"Calculated trajectories. Trajectories: "<<trajList.size()<<" Each: "<<trajList[0].pos.size()<<endl;
    return trajList;
}
...
Trajectory Solver::calculateTrajectory(vector<double> coef, double t0, double t1) {
    ...
    cout<<"Calculated XYZ trajectories"<<endl;
    return traj;
}
